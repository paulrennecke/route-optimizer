...
const MapHandler = (() => {
    ...
    return {
        ...
        geocodeAddresses: async function(addresses) {
            const geocoder = new google.maps.Geocoder();
            const extractAddress = val => /^(.+)\s+—\s+(.+)$/.exec(val)?.[2]?.trim() || val.trim();
            const geocodeAddress = address => new Promise((resolve, reject) => {
                geocoder.geocode({ address: extractAddress(address) }, (results, status) => {
                    status === google.maps.GeocoderStatus.OK
                        ? resolve({ address, location: results[0].geometry.location, placeId: results[0].place_id })
